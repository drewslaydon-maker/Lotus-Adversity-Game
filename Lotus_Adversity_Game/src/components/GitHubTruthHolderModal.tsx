...
interface GitHubTruthHolderModalProps {
  isOpen: boolean;
  onClose: () => void;
}

export const GitHubTruthHolderModal: React.FC<GitHubTruthHolderModalProps> = ({
  isOpen,
  onClose,
}) => {
  ...
  const [repoUrl, setRepoUrl] = useState<string>(() => {
    return localStorage.getItem("apparatus_gh_repo") || "brokenelysium/Lotus_Adversity_Game";
  });
  const [token, setToken] = useState<string>(() => {
    return localStorage.getItem("apparatus_gh_token") || "";
  });
  const [branch, setBranch] = useState<string>(() => {
    return localStorage.getItem("apparatus_gh_branch") || "main";
  });
  const [commitMessage, setCommitMessage] = useState<string>(
    "feat(apparatus): Alden North wheel matrix & roadmap status verified"
  );
  ...
  // Active subtab in modal: "push" | "mac"
  const [activeSubtab, setActiveSubtab] = useState<"push" | "mac">("push");

  useEffect(() => {
    if (repoUrl) localStorage.setItem("apparatus_gh_repo", repoUrl);
  }, [repoUrl]);

  useEffect(() => {
    if (token) localStorage.setItem("apparatus_gh_token", token);
  }, [token]);

  useEffect(() => {
    if (branch) localStorage.setItem("apparatus_gh_branch", branch);
  }, [branch]);
  ...
  const handlePushToGitHub = async () => {
    if (!token.trim()) {
      setPushStatus("error");
      setStatusMessage("Please enter your GitHub Personal Access Token (PAT).");
      playSfx("anvil");
      return;
    }
    ...
    if (!repoUrl.trim()) {
      setPushStatus("error");
      setStatusMessage("Please specify your repository (e.g. username/repo-name).");
      playSfx("anvil");
      return;
    }
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        setPushStatus("error");
        setStatusMessage(data.error || "Failed to push to GitHub repository.");
        setLogOutput(data.details || "");
        playSfx("anvil");
      }
    } catch (err: any) {
      setPushStatus("error");
      setStatusMessage(err?.message || "Network error while connecting to sync engine.");
      playSfx("anvil");
    } finally {
      setIsPushing(false);
    }
      ...
      >
        ...
        <div className="p-4 sm:p-5 border-b border-neutral-800 flex items-center justify-between gap-3 bg-neutral-900/60">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-neutral-400 font-sans">
                Push directly to your repository with zero Mac terminal commands.
              </p>
            </div>
          </div>
          ...
          <button
            ...
            className="p-2 rounded-lg bg-neutral-900 border border-neutral-800 text-neutral-400 hover:text-neutral-200 hover:bg-neutral-850 transition-colors cursor-pointer"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="px-5 pt-3 border-b border-neutral-800 flex items-center gap-3 bg-neutral-950">
          ...
          >
            ...
            <span>1-Click Push to GitHub (Option 1)</span>
          </button>
          ...
          >
            ...
            <span>Local Files on Mac (Option 2)</span>
          </button>
        </div>
        ...
        <div className="p-5 space-y-4 overflow-y-auto font-sans text-xs">
          ...
          {activeSubtab === "push" ? (
            <div className="space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  placeholder="e.g. brokenelysium/Lotus_Adversity_Game"
              ...
              <div className="space-y-1.5">
                ...
                <p className="text-[11px] text-neutral-400 leading-relaxed">
                  Saved safely in your browser session. The token is never stored in git plain-text config files.
                </p>
              </div>

              {/* Branch and Message Row */}
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div className="space-y-1 sm:col-span-1">
                  <label className="font-mono text-[11px] font-bold text-neutral-300 block">
                    Branch
                  </label>
                  <input
                    type="text"
              ...
              {/* Force Push Toggle (Optional recovery) */}
              <div className="flex items-center justify-between p-3 rounded-xl bg-neutral-900/40 border border-neutral-800">
                <div>
                  <span className="font-mono font-bold text-neutral-300 text-xs block">
                    Force Push Override
                  </span>
                  <span className="text-[11px] text-neutral-500">
                    Use if the remote repository has divergent history or an empty README
                  </span>
                </div>
                <input
                  type="checkbox"
              ...
              >
                {isPushing ? (
                  <>
                    ...
                    <span>Executing Push to GitHub...</span>
                  </>
                ) : (
                  <>
                    <GitBranch className="w-4 h-4" />
                    <span>Push Current Engine State to GitHub</span>
                  </>
                )}
              </button>
            ...
            <div className="space-y-4">
              ...
              {/* GitHub Desktop Workflow Guide for Mac */}
              <div className="p-4 rounded-xl bg-neutral-900/70 border border-neutral-800 space-y-3">
                <div className="flex items-center gap-2 text-xs font-mono font-bold text-amber-400">
                  <Laptop className="w-4 h-4" />
                  <span>MAC WORKFLOW: GITHUB DESKTOP (ZERO TERMINAL COMMANDS)</span>
                </div>

                <div className="space-y-2.5 text-xs text-neutral-300 leading-relaxed font-sans">
                  <div className="flex items-start gap-2.5 p-2 rounded-lg bg-neutral-950/70 border border-neutral-850">
                    ...
                    <div>
                      <strong className="text-neutral-200">Push here in the web app:</strong> Use the "1-Click Push" tab to send your ratified files to your GitHub repository.
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-2.5 p-2 rounded-lg bg-neutral-950/70 border border-neutral-850">
                    ...
                    <div>
                      <strong className="text-neutral-200">Open GitHub Desktop on Mac:</strong> Go to <span className="font-mono text-amber-300">File &gt; Clone Repository...</span> and select <span className="font-mono text-neutral-200">{repoUrl}</span>.
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-2.5 p-2 rounded-lg bg-neutral-950/70 border border-neutral-850">
                    ...
                    <div>
                      <strong className="text-neutral-200">Automatic Sync:</strong> All files appear on your Mac in Finder. Any time updates occur, just click <span className="font-mono text-emerald-400 font-bold">"Fetch origin / Pull"</span> in GitHub Desktop. No terminal required!
                    </div>
                  </div>
                </div>
              </div>

              {/* Instant Standalone ZIP Download for Mac */}
              <div className="p-4 rounded-xl bg-neutral-900/50 border border-amber-500/30 space-y-3">
                ...
                <p className="text-xs text-neutral-400 leading-relaxed">
                  Want the entire project directly on your Mac right now without even using GitHub? 
                  Download the complete raw source code archive (.zip) to extract directly in Finder.
                </p>

                <button
                  onClick={handleDownloadZip}
                  className="w-full py-3 px-4 rounded-xl bg-neutral-800 hover:bg-neutral-750 border border-neutral-700 text-neutral-100 font-mono text-xs font-bold flex items-center justify-center gap-2 transition-all cursor-pointer shadow-md"
                >
